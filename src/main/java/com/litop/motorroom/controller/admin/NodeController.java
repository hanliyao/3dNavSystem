...
public class NodeController extends BaseController{
 ...
 NodeMapper nodeDao;
  ...
  public String index(Model model,HttpServletRequest request,HttpServletResponse response) {
     ...
     // int CurrentUserID = (Integer) session.getAttribute("litopAdminId");
          ...
          List<Node> list=nodeDao.selectNodeListPage(paramMap);

          //总共多少条记录
          Map<String, Object> countMap = new HashMap<String, Object>();
          countMap.put("moduleName", moduleName);
          int totalCount = nodeDao.getCounts(countMap);
    ...
    public String preInsert(Model model) {
    	//获取属于第一级的节点列表
		List<Node> list = nodeDao.getListByAuthType(0);
		model.addAttribute("nodeList", list);
    ...
    public String insert(@Param("node") Node node) {
    ...
    public String update(@Param("node") Node node) {
        nodeDao.updateByPrimaryKey(node);
        return "redirect:/node/index";
    }
